...
??? note "Complete file"
    ...
    === "database.py :fontawesome-brands-python:"
...
def main(max_sec: int = 3600 * 2):
    ...
    for _ in range(0, max_sec, 60):
        ...
        result: Tokens = session.query(Tokens)
                                .filter(Tokens.access_token_expires <= now)
                                .first()
        
        if result:
            # change status to updating
            session.query(Tokens)
                .filter(Tokens.id == result.id)
                .update(values={"updating": True})
            # get new set of tokens
            tokens, data = get_new_refresh_token(
...
=== "**Terminal 1: :fontawesome-brands-python: refresh_token_worker.py**"
...
=== "**Terminal 2: :fontawesome-brands-python: access_token_consumer.py**"
