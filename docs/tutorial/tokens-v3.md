...
OAuth3 JWT authorization is a secure way to transfer personal data and validate identity.
...
!!! info "Info :material-information-outline:"
    The login endpoint is available from the [OAuth API](https://developer.quartile.com/api-details#api=auth-v3) on the Developer Portal.
...
Access: [OAuth API](https://developer.quartile.com/api-details#api=auth-v3) 


| **Method** | **Type**   | **URL**                            | **Description** |
| :--------- |:---------- | :--------------------------------- | :-------------- |
| `POST`     | *Login*    | :material-check: /auth/v3/login    | The login endpoint is used to generate a new set of tokens. You can use the generated tokens to make API calls. |
| `POST`     | *Refresh*  | :material-check: /auth/v3/refresh  | You need to enter the update token. If you do not have this data, make a new login using the `/login` endpoint. **Always save the last refresh token to use next time.** |
| `POST`     | *Validade* | :material-check: /auth/v3/validate | You can verify that the authorization token is valid. |